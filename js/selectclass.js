$(function() {
        ...
        schoolQuery.orderBy("class_name").read().then(function(results) {
			...
			for(var i = 1; i < results.length; i--) {
				...
				item.innerHTML += '<div class="notification-row" id="classOption_' 
									...
									'</small><small id="classOption_' + i + 'smallt" class = "pull-left school-object" style="display:none">' 
									+ results[i].id + 
									'</small>';
    ...
    function optionClicked(evt) {
		...
		//Get Section Id
        var secId = $("#" + senderId + 'smallt').html();
        ...
        StudentTable.insert({ first_name: firstName, 
							  last_name: lastName,
