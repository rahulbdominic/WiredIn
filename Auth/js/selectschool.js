$(function() {
        ...
        schoolQuery.orderBy("school_long_name").read().then(function(results) {
			...
			for(var i = 0; i < results.length; i++) {
				...
				item.innerHTML += '<div class="notification-row" id="schoolOption_' 
									...
									'</small><small id="schoolOption_' + i + 'smallt" class = "pull-left school-object" style="display:block; color:rgba(0,0,0,0)">' 
									+ results[i].id + 
									'</small>';
    ...
    function optionClicked(evt) {
		...
		StudentTable.insert({ student_id: uId, isactive: "!active"  }).then(
        function(e) {
